  ...
  return (
    <div className="min-h-screen bg-base-950 text-slate-100 print:min-h-0 print:bg-white">
      ...
      <div className="mx-auto flex w-full max-w-[160mm] justify-center px-4 py-10 print:max-w-none print:px-0 print:py-0">
        <article className="flyer-sheet card-print relative flex h-[210mm] w-[148mm] shrink-0 flex-col overflow-hidden bg-indigo-950 text-slate-100 shadow-soft print:rounded-none print:shadow-none">
          {/* Fondo decorativo con el mismo lenguaje visual que las tarjetas. */}
          <div
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 bg-radial-premium opacity-90"
          />
          <div
            aria-hidden="true"
          ...
          <div className="relative flex h-full flex-col px-8 py-9">
            ...
            <section className="mt-6">
              <p className="inline-flex rounded-full border border-blue-200/20 bg-blue-400/10 px-3 py-1 font-display text-[9px] font-semibold uppercase tracking-[0.18em] text-blue-100">
                Nuestros servicios
              </p>
              <ul className="mt-3 grid grid-cols-2 gap-2.5">
                {highlightedSolutions.map((solution) => (
                  <li
                    key={solution.title}
                    className="flex items-start gap-2 rounded-lg border border-white/10 bg-white/5 p-2.5"
                  >
                    ...
                    <div>
                      <p className="text-[11px] font-semibold text-white">{solution.title}</p>
            ...
            <section className="mt-auto pt-6">
              <div className="flex items-end justify-between gap-4 rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="min-w-0">
                  <p className="font-display text-sm font-semibold text-white">{card.name}</p>
                  <p className="mt-1 inline-flex rounded-full border border-blue-200/20 bg-blue-400/10 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.14em] text-blue-100">
                    {card.role}
                  </p>
                ...
                <div className="flex shrink-0 flex-col items-center gap-1">
                  ...
                  />
                  <span className="text-center text-[8px] uppercase tracking-[0.14em] text-slate-300">
                    WhatsApp
                  </span>
                </div>
              </div>
